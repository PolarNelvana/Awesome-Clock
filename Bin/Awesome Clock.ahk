...
settingsFile = %A_AppData%\Awesome\Settings.ini
...
{
	IniRead, initWinX, %SettingsFile%, Position, WinX, %initWinX%
	IniRead, initWinY, %SettingsFile%, Position, WinY, %initwiny%
	IniRead, ParentDesktop, %SettingsFile%, Position, ParentDesktop, %ParentDesktop%
	IniRead, Lock, %SettingsFile%, Position, Lock, %Lock%
	IniRead, Snapnum, %SettingsFile%, Position, Snap, %Snap%
	ifequal, snapnum, 1
	Snap = Nothing
	ifequal, snapnum, 2
	snap = Top Right
	...
	IniRead, UseSysAccent, %SettingsFile%, Colors, UseSysAccent, %UseSysAccent%
}
else
filecreatedir, %a_appdata%\Awesome Clock
initaccent := colorSecondHand
gosub, snapconfig
gosub, accentfig
...
menu, mainmenu, add, Exit, exitsub
...
Gui, 2:Add, Text, x10 y0 w210 h40, Settings
Gui, 2:Font, s12
Gui, 2:Add, Progress, x0 y40 w225 h1 BackgroundFFAA00, 0

SetFormat, IntegerFast, Hex

Temp1 := Color1 & 0xFFFFFF
Gui, 2:Add, Text, x10 y52 w80 h20 gSetColor, 1
Gui, 2:Add, Progress, x10 y52 w80 h20 vTempColor1 Background%Temp1%
Gui, 2:Add, Text, x97 y51 w100 h20, Element 1

Temp1 := Color2 & 0xFFFFFF
Gui, 2:Add, Text, x10 y83 w80 h20 gSetColor, 2
Gui, 2:Add, Progress, x10 y83 w80 h20 vTempColor2 Background%Temp1%
Gui, 2:Add, Text, x97 y82 w100 h20, Element 2

Temp1 := Color3 & 0xFFFFFF
Gui, 2:Add, Text, x10 y114 w80 h20 gSetColor, 3
Gui, 2:Add, Progress, x10 y114 w80 h20 vTempColor3 Background%Temp1%
Gui, 2:Add, Text, x97 y113 w100 h20, Element 3

Temp1 := Color4 & 0xFFFFFF
Gui, 2:Add, Text, x10 y145 w80 h20 gSetColor, 4
Gui, 2:Add, Progress, x10 y145 w80 h20 vTempColor4 Background%Temp1%
Gui, 2:Add, Text, x97 y144 w100 h20, Element 4

Temp1 := Color5 & 0xFFFFFF
Gui, 2:Add, Text, x10 y176 w80 h20 gSetColor, 5
Gui, 2:Add, Progress, x10 y176 w80 h20 vTempColor5 Background%Temp1%
Gui, 2:Add, Text, x97 y175 w100 h20, Element 5

Temp1 := Color6 & 0xFFFFFF
Gui, 2:Add, Text, x10 y207 w80 h20 gSetColor, 6
Gui, 2:Add, Progress, x10 y207 w80 h20 vTempColor6 Background%Temp1%
Gui, 2:Add, Text, x97 y206 w100 h20, Element 6

Temp1 := ColorHand & 0xFFFFFF
Gui, 2:Add, Text, x10 y238 w80 h20 gSetColor, Hand
Gui, 2:Add, Progress, x10 y238 w80 h20 vTempColorHand Background%Temp1%
Gui, 2:Add, Text, x97 y237 w100 h20, Hands

Temp1 := ColorSecondHand & 0xFFFFFF
Gui, 2:Add, Text, x10 y269 w80 h20 gSetColorsec, SecondHand
Gui, 2:Add, Progress, x10 y269 w80 h20 vTempColorSecondHand Background%Temp1%
Gui, 2:Add, Text, x97 y268 w100 h20, Second Hand

ifequal, parentdesktop, 1
Gui, 2:Add, Checkbox, x10 y297 w200 h20 vparentdesktop gToggleParent checked, Parent To Desktop
else
Gui, 2:Add, Checkbox, x10 y297 w200 h20 vparentdesktop gToggleParent, Parent to Desktop
ifequal, lock, 1
gui, 2:Add, checkbox, x10 y321 w200 h20 vlock gtogglelock checked, Lock Position
else
gui, 2:Add, checkbox, x10 y321 w200 h20 vlock gtogglelock, Lock Position
ifequal, usesysaccent, 1
gui, 2:add, checkbox, x10 y345 w200 h20 checked vusesysaccent gtoggleaccent, Use System Accent
else
gui, 2:add, checkbox, x10 y345 w200 h20 vusesysaccent gtoggleaccent, Use System Accent
gui, 2:add, dropdownlist, x10 y373 w120 h20 r3 vsnap gmover choose%snapnum%, Nothing|Top Right|Bottom Right|Top Left|Bottom Left|Center
gui, 2:add, text, x136 y376 w100 h20, Snap To

Gui, 2:Add, CheckBox, x211 y52 w25 h20
Gui, 2:Add, CheckBox, x211 y83 w25 h20
Gui, 2:Add, CheckBox, x211 y114 w25 h20
Gui, 2:Add, CheckBox, x211 y145 w25 h20
Gui, 2:Add, CheckBox, x211 y176 w25 h20
Gui, 2:Add, CheckBox, x211 y207 w25 h20
Gui, 2:Add, CheckBox, x211 y238 w25 h20
Gui, 2:Add, CheckBox, x211 y269 w25 h20


menu, contextmenu, add, Reset, resetter
GuiControl, 2:Focus, TempColor1 ;remove focus from the control

SetFormat, IntegerFast, D

Gui, 2:+AlwaysOnTop +ToolWindow
Gui, 2:Show, w255 h413 Hide, Awesome Clock
...
Ifequal, Lock, 0
{
	hSysMenu:=DllCall("GetSystemMenu","Int",CLK,"Int",TRUE) 
	nCnt:=DllCall("GetMenuItemCount","Int",hSysMenu) 
	DllCall("RemoveMenu","Int",hSysMenu,"UInt",nCnt-6,"Uint","0x400") 
	DllCall("DrawMenuBar","Int",CLK)
}
else
{
	hSysMenu:=DllCall("GetSystemMenu","Int",CLK,"Int",FALSE) 
	nCnt:=DllCall("GetMenuItemCount","Int",hSysMenu) 
	DllCall("RemoveMenu","Int",hSysMenu,"UInt",nCnt-6,"Uint","0x400") 
	DllCall("DrawMenuBar","Int",CLK)
}
...
catch
{
	ifexist, %settingsfile%
	{
		msgbox, 0x10, Error, Failed to reset the settings.
		return
	}
}
onexit, exit
...
snapconfig:
; Nothing|Top Right|Bottom Right|Top Left|Bottom Left|Center
ifequal, snap, Nothing
{
	if (WinX = initWinX) and (WinY = initWinY)
	return
	if (WinX != initWinX) or (WinY != initWinY)
	{
		WinX := initWinX
		WinY := initWinY
	}
}
ifequal, snap, Top Right
{
	winX := A_ScreenWidth - 515
	winY := 0
}
ifequal, snap, Bottom Right
{
	winX := A_ScreenWidth - 515
	winY := A_ScreenHeight - 515
}
ifequal, snap, Top Left
{
	WinX := 0
	WinY := 0
}
ifequal, snap, Bottom Left
{
	winX := 0
	winY := A_ScreenHeight - 515
}
...
WinMove, ahk_id %CLK%,, %winx%, %winy%
return

accentchang:
ifnotequal, usesysaccent, 1
return
else
{
	gosub, accentfig
	SetColor("SecondHand",ColorSecondHand)
	GuiControl, 2:+Background%SysAccentBack%, TempColorSecondHand
}
return
...
{
	try
	RegRead, SysAccent, HKCU\SOFTWARE\Microsoft\Windows\DWM, ColorizationColor
	stringlen, length, sysaccent
	ifnotequal, length, 10
	{
		IniRead, SysAccent, %SettingsFile%, Colors, LastKnownConfig, ERROR
		ifequal, SysAccent, ERROR
		return
		else
		RelyingOnLKC = True
	}
	...
	RelyingOnLKC =
	ColorSecondHand := SysAccent
	A_FormatInteger1 := A_FormatInteger
	SetFormat, IntegerFast, Hex
	SysAccentBack := SysAccent+0
	StringRight, SysAccentBack, SysAccentBack, 6
	SetFormat, IntegerFast, %A_FormatInteger1%
	IniWrite, %ColorSecondHand%, %SettingsFile%, Colors, LastKnownConfig
}
else
{
	ColorSecondHand := initaccent
	A_FormatInteger1 := A_FormatInteger
	SetFormat, IntegerFast, Hex
	SysAccentBack := ColorSecondHand+0
	StringRight, SysAccentBack, SysAccentBack, 6
	SetFormat, IntegerFast, %A_FormatInteger1%
	SetColor("SecondHand",ColorSecondHand)
	GuiControl, 2:+Background%SysAccentBack%, TempColorSecondHand
...
ifequal, usesysaccent, 1
{
	tooltip You selected to use the system accent`, so this button is disabled!
...
If (ChosenColor != "")
{
	...
	ifequal, A_GuiControl, SecondHand
	initaccent := temp1
}
SetFormat, IntegerFast, D
Gui, 2:-Disabled
...
ChooseColor(InitialColor = 0x000000,hWindow = 0)
{
	...
	A_FormatInteger1 := A_FormatInteger
	SetFormat, IntegerFast, Hex
	ChosenColor := NumGet(Dialog,12)
	ChosenColor := SubStr("00000" . SubStr(((ChosenColor & 0xFF) << 16) | (ChosenColor & 0xFF00) | ((ChosenColor & 0xFF0000) >> 16),3),-5)
...
IniWrite, %Lock%, %SettingsFile%, Position, Lock
; Nothing|Top Right|Bottom Right|Top Left|Bottom Left|Center
ifequal, Snap, Nothing
IniWrite, 1, %SettingsFile%, Position, Snap
ifequal, Snap, Top Right
IniWrite, 2, %SettingsFile%, Position, Snap
ifequal, Snap, Bottom Right
IniWrite, 3, %SettingsFile%, Position, Snap
ifequal, Snap, Top Left
IniWrite, 4, %SettingsFile%, Position, Snap
ifequal, Snap, Bottom Left
IniWrite, 5, %SettingsFile%, Position, Snap
ifequal, Snap, Center
IniWrite, 6, %SettingsFile%, Position, Snap
...
IniWrite, %UseSysAccent%, %SettingsFile%, Colors, UseSysAccent
ifnotequal, usesysaccent, 1
IniWrite, %ColorSecondHand%, %SettingsFile%, Colors, ColorSecondHand
else
IniWrite, %initaccent%, %SettingsFile%, Colors, ColorSecondHand
return
